...
async def verify_api_key(
    ...
    Authentication format: "Authorization: Bearer <api_key>"

    Users must:
    1. Generate an API key from OpenHands Settings -> API Keys
    2. Configure Azure DevOps Service Hook with HTTP Header:
       Name: Authorization
       Value: Bearer <api_key>

    Args:
        authorization: Authorization header value from FastAPI Header()
        request: Optional request object to extract header directly if needed
    ...
    """
    # If authorization is None, try to extract directly from request headers
    if not authorization and request:
        authorization = request.headers.get('Authorization')
        if authorization:
            logger.debug(
                '[Azure DevOps Webhook] Extracted Authorization header directly from request'
            )
    ...
    if not authorization:
        ...
        raise HTTPException(
            status_code=401,
            detail='Unauthorized: Missing Authorization header. Configure webhook with HTTP Header - Name: "Authorization", Value: "Bearer <api_key>"',
        )

    # Only support Bearer token format
    if not authorization.startswith('Bearer '):
        logger.warning(
            '[Azure DevOps Webhook] Invalid Authorization header format (expected Bearer token)'
        )
        raise HTTPException(
            status_code=401,
...
async def azure_devops_events(
    ...
    try:
        ...
        try:
            user_id = await verify_api_key(authorization, request)
            logger.info(
